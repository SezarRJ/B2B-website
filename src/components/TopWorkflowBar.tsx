        ...
        {stages.map((st, idx) => {
          ...
          return (
            <React.Fragment key={st.id}>
              ...
              >
                <span
                  className={`px-1.5 py-0.5 rounded text-[10px] font-black ${isActive ? "bg-black text-white" : "bg-slate-800 text-slate-400"}`}
                >
                  {st.step}
                </span>
              ...
              {idx < stages.length - 1 && (
                <ChevronRight
                  className={`h-4 w-4 flex-shrink-0 ${isPassed ? "text-yellow-500" : "text-slate-700"}`}
                />
              )}
            </React.Fragment>
          );
