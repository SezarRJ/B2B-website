...
function DemandsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess(
        t(
          "demands.post.success",
          "Commercial inquiry posted successfully. Distributed match indexing active.",
        ),
      );
    } catch (err: any) {
    ...
    >
      ...
      <main className="flex-1 overflow-auto">
        <header className="flex h-16 items-center justify-between border-b border-border bg-white px-6 lg:px-8 select-none">
          ...
          >
            ...
            <span>
              {showForm
                ? t("btn.submit", "Cancel")
                : t("Post Purchasing Inquiry", "Post Purchasing Inquiry")}
            </span>
          </Button>
        </header>
        ...
        <div className="p-6 lg:p-8 max-w-6xl mx-auto space-y-6">
            ...
            <CardContent className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div className="space-y-1.5 max-w-2xl select-text">
                <Badge className="bg-amber-500 text-black font-extrabold uppercase font-mono tabular-nums text-[10px] tracking-widest">
                  Workflow Trajectory Handshake Handoff
                </Badge>
                <h2 className="text-lg font-extrabold text-foreground font-sans">
                  Advance to Stage 02: Launch Smart Spot Matching
                </h2>
                <p className="text-xs text-muted-foreground leading-relaxed font-sans">
                  Once your target corporate purchasing inquiries are active below, trigger our
                  automated smart matching sidecar. The engine actively correlates purchasing
                  volumes against verified international spot catalogs (/products) and generates
              ...
              >
                <Sparkles
                  className="h-5 w-5 fill-black animate-spin"
                  style={{ animationDuration: "6s" }}
                />
                <span>
                  {generating ? "Running Match Algorithms..." : "🤝 Run Stage 02 Match Engine"}
                </span>
                <ChevronRight
                  className={
                    isRtl
          ...
          {success && (
            <p className="text-sm font-bold text-green-600 bg-green-50 p-3 rounded-lg border border-green-200">
              {success}
            </p>
          )}
          {error && (
          ...
          {/* New Demand Form */}
          {showForm && (
            <Card className="border-2 border-primary/40 bg-white shadow-lg font-mono tabular-nums">
              <CardHeader>
                <CardTitle className="text-base font-extrabold text-foreground font-sans">
                  Create Structured B2B Purchasing Manifest
                </CardTitle>
              </CardHeader>
              <CardContent>
                <form
                  onSubmit={handleSubmit}
                  className="space-y-4 text-xs font-medium font-mono tabular-nums"
                >
                  <div className="grid md:grid-cols-3 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="productName" className="font-bold font-sans">
                      ...
                      <Input
                        ...
                        placeholder="e.g., Standard Basra Bulk Dates"
                        required
                      />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="category" className="font-bold font-sans">
                        Category Code
                      </Label>
                      <Input
                        id="category"
                        ...
                        placeholder="e.g., dates or steel_scrap"
                        required
                      />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="quantity" className="font-bold font-sans">
                        Purchasing Tonnage
                      </Label>
                      <Input
                        id="quantity"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="unit" className="font-bold font-sans">
                        Standard Unit
                      </Label>
                      <Input
                        id="unit"
                        ...
                        placeholder="e.g., ton or container"
                        required
                      />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="budget" className="font-bold font-sans">
                        Max Target FOB Budget ($ / unit)
                      </Label>
                      <Input
                        id="budget"
                        type="number"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="location" className="font-bold font-sans">
                        Destination Free Zone or Port
                      </Label>
                      <Input
                        id="location"
                        value={location}
                        onChange={(e) => setLocation(e.target.value)}
                        placeholder="e.g., Mersin Bonded Warehouse, Turkey"
                        required
                      />
                    </div>
                  </div>
                  ...
                  <Button
                    type="submit"
                    className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-3 mt-2 font-sans"
                    disabled={submitting}
                  >
                    {submitting
                      ? "Broadcasting purchasing requirements..."
                      : "Authorize Purchasing Inquiry"}
                  </Button>
                </form>
              </CardContent>
            </Card>
            ...
            {loading ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {demands.map((demand) => (
                  <Card
                    key={demand.id}
                    className="overflow-hidden border border-border bg-white shadow-sm hover:border-primary/50 transition-colors flex flex-col justify-between"
                  >
                    <CardHeader className="p-5 pb-3 bg-secondary/30 border-b border-border">
                      ...
                      <h3 className="font-extrabold text-foreground text-base pt-2">
                        {demand.product_name}
                      </h3>
                    </CardHeader>
                    ...
                    <CardContent className="p-5 space-y-4 flex-1 flex flex-col justify-between">
                      <div className="space-y-2 text-xs font-mono tabular-nums">
                        <div className="flex items-center justify-between p-2 rounded bg-secondary/40">
                          ...
                          <span className="font-black text-foreground">
                            {demand.quantity} {demand.unit.toUpperCase()}
                          </span>
                        </div>
                        <div className="flex items-center justify-between p-2 rounded bg-primary/10 text-primary">
                          <span className="font-sans font-bold">Max Target Budget:</span>
                          <span className="font-black">
                            ${Number(demand.budget).toFixed(2)} / {demand.unit}
                          </span>
                        </div>
                      ...
                      <div className="pt-4 border-t border-border flex justify-between items-center text-[10px] text-muted-foreground font-mono tabular-nums">
                        <span>
                          Urgency SLA: {demand.urgency ? `${demand.urgency}x Priority` : "Standard"}
                        </span>
                        <Button
                          size="sm"
                          className="bg-primary hover:bg-primary/90 text-white font-sans font-bold h-8 text-xs"
