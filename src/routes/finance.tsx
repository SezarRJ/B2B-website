      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          ...
          <div className="bg-surface-50 rounded-xl p-3">
            ...
            <p className="text-sm font-bold text-surface-800">
              {new Date(lc.expiry_date).toLocaleDateString()}
            </p>
          </div>
        </div>
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-surface-400">
              Step {currentIndex + 1} of {lcStatusSteps.length}
            </span>
          </div>
            ...
            {lcStatusSteps.map((step, idx) => {
              ...
              return (
                <div key={step.id} className="flex-1 flex flex-col items-center">
                  ...
                  />
                  <p
                    className={`text-[10px] font-medium text-center ${isCompleted ? "text-indigo-700" : "text-surface-400"}`}
                  >
                    {step.label}
                  </p>
                </div>
...
function DPWorkflow({ dp, onUpdate }: { dp: DocumentaryCollection; onUpdate: () => void }) {
  ...
  const nextAction =
    dp.status === "presented_to_importer" ? "pay" : dp.status === "paid" ? "send" : null;
      ...
      <div className="p-5">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-surface-400">
              Step {currentIndex + 1} of {dpStatusSteps.length}
            </span>
          </div>
          <div className="flex items-center gap-1">
            {dpStatusSteps.map((step, idx) => {
              ...
              return (
                <div key={step.id} className="flex-1 flex flex-col items-center">
                  ...
                  />
                  <p
                    className={`text-[10px] font-medium text-center ${isCompleted ? "text-accent-700" : "text-surface-400"}`}
                  >
                    {step.label}
                  </p>
                </div>
    ...
    <div className="space-y-6 animate-slide-in">
      ...
      <div>
        ...
        <p className="text-sm text-surface-500 mt-0.5">
          Manage Letters of Credit and Documentary Collections
        </p>
      </div>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-surface-200">
          ...
          <p className="text-xs text-surface-500 leading-relaxed">
            Most secure payment method. Bank guarantees payment upon presentation of compliant
            documents.
          </p>
        </div>
        <div className="bg-white rounded-2xl p-5 border border-surface-200">
        ...
        lcs.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 border border-surface-200 text-center">
            ...
            <p className="text-sm text-surface-500">
              L/Cs will appear here when created for orders
            </p>
          </div>
        ) : (
