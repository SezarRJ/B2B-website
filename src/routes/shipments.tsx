        ...
        <header className="flex h-16 items-center justify-between border-b border-border bg-white px-6 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold text-foreground">
              Logistics Container Tracking & GPS Waypoints
            </h1>
          </div>
          <Badge
            variant="outline"
            className="border-primary text-primary bg-primary/5 font-mono tabular-nums"
          >
            ...
            <CardHeader className="pb-3">
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                  ...
                  <CardDescription>
                    Monitor physical cross-border freight trajectories. The platform consumes
                    automated EDI tracking webhooks from shipping lines, updating commercial escrow
                    custody locks upon customs clearing.
                  </CardDescription>
                </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              {loading ? (
                <p className="text-muted-foreground text-sm">
                  Querying global freight satellites...
                </p>
              ) : shipments.length === 0 ? (
                <Card className="p-12 text-center bg-secondary/30">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Accept a deal and generate an Order to create an active container tracking
                    manifest.
                  </p>
                </Card>
              ) : (
                    ...
                    >
                      <CardHeader className="bg-secondary/40 border-b border-border pb-4">
                        <div className="flex flex-wrap items-center justify-between gap-4">
                          <div className="flex items-center gap-3">
                            <Badge className="font-mono tabular-nums text-xs bg-primary text-white">
                            ...
                            <Badge
                              variant={ship.status === "delivered" ? "outline" : "secondary"}
                              className={
                      ...
                      <CardContent className="p-6 space-y-6">
                        ...
                        <div className="grid md:grid-cols-2 gap-4 p-4 bg-secondary/30 rounded-xl text-xs font-mono tabular-nums">
                          <div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-1.5">
                            {shipmentStatuses.map((st, idx) => {
                              const isCompleted =
                                idx <= (currentStatusIdx >= 0 ? currentStatusIdx : 0);
                              ...
                              return (
                                ...
                                >
                                  <span className="text-[10px] block font-mono tabular-nums">
                                    Stage {idx + 1}
                                  </span>
                                  <span className="text-[11px] tracking-tight block truncate">
                                    {st.label}
                                  </span>
                                </div>
                              );
                        ...
                        <div className="space-y-3 pt-2 border-t border-border">
                          <span className="text-[11px] font-bold text-foreground uppercase tracking-wider block font-mono tabular-nums flex items-center gap-1.5">
                          ...
                          <div className="space-y-2.5 max-h-60 overflow-y-auto pr-1">
                            {ship.events.map((ev, evtIdx) => (
                              <div
                                key={ev.id || evtIdx}
                                className="flex items-start gap-3 p-3 rounded-xl bg-secondary/30 border border-border/80 text-xs"
                              >
                                <div className="p-1 rounded bg-primary/10 text-primary mt-0.5 flex-shrink-0">
                                  <MapPin className="h-3.5 w-3.5" />
                                </div>
                                <div className="space-y-0.5 flex-1 font-mono tabular-nums">
              ...
              <CardContent className="space-y-4">
                <form onSubmit={handleInjectCheckpoint} className="space-y-4">
                    ...
                    <Select value={nextStatus} onValueChange={setNextStatus}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="out_for_delivery">
                          Final Haulage (Out for Delivery)
                        </SelectItem>
                  ...
                  <Button
                    ...
                    disabled={injecting || !targetShipmentId}
                  >
                ...
                <div className="pt-4 border-t border-border space-y-2">
                  <span className="text-[11px] font-bold text-foreground block">
                    Quick Autopopulate Targets:
                  </span>
                  <Button
                    size="sm"
                    variant="outline"
                    className="w-full justify-start text-xs border-blue-300 hover:bg-blue-50 font-mono tabular-nums"
                    onClick={() =>
                      autofillSample(
                        "Suez Canal / Eastern Med Maritime Waypoint",
                        "Vessel transiting restricted corridor seamlessly. Military escort accompanying fleet.",
                        "in_transit",
                      )
                    }
                  >
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    className="w-full justify-start text-xs border-amber-300 hover:bg-amber-50 font-mono tabular-nums"
                    onClick={() =>
                      autofillSample(
                        "Mersin Free Zone Import Customs Desk",
                        "Cargo unloaded to bonded warehouse. Commercial invoice and certificate of origin authenticated.",
                        "customs_import",
                      )
                    }
                  >
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    className="w-full justify-start text-xs border-green-300 hover:bg-green-50 font-mono tabular-nums"
                    onClick={() =>
                      autofillSample(
                        "Istanbul Importer Industrial Facility",
                        "Physical commodity delivered. 100% Quality verification confirmed by SGS field inspector.",
                        "delivered",
                      )
                    }
                  >
