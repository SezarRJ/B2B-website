        ...
        <header className="flex h-16 items-center justify-between border-b border-border bg-white px-6 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold text-foreground">
              Compliance & Sanctions Screening Gateway
            </h1>
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-foreground">
                    Global Anti-Money Laundering & SDN Matcher
                  </h2>
                  <p className="text-xs text-muted-foreground mt-1 max-w-3xl leading-relaxed">
                    Prior to executing escrow deposits or final export documentation, all involved
                    trading entities (buyers, sellers, logistics carriers) are cross-referenced
                    against Specially Designated Nationals (SDN) and automated consolidated embargo
                    databases.
                  </p>
                </div>
              </div>
                  ...
                  <CardContent className="space-y-6">
                    <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      >
                        ...
                        {loading
                          ? "Cross-referencing global indices..."
                          : "Execute Cryptographic Compliance Sweep"}
                      </Button>
                    </form>
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <p className="font-bold text-base">
                              {result.match_found
                                ? "RESTRICTED MATCH HIT (P0)"
                                : "Entity Fully Cleared (No Hit)"}
                            </p>
                            <Badge
                              variant={result.match_found ? "destructive" : "outline"}
                              className={
                                !result.match_found
                          ...
                          <p className="text-sm pt-1 leading-relaxed">
                            {result.match_details ||
                              "No adverse media or sanctions records identified."}
                          </p>
                        </div>
                      </div>
                    )}
                  ...
                  <CardContent className="space-y-3">
                    <div className="space-y-2">
                      <p className="text-xs font-bold text-foreground">
                        Simulate Positive Sanctions Hit:
                      </p>
                      <Button
                        size="sm"
                        variant="outline"
                        className="w-full justify-start text-xs border-red-300 hover:bg-red-50 hover:text-red-700 font-mono tabular-nums"
                    ...
                    <div className="space-y-2 pt-2 border-t border-border">
                      <p className="text-xs font-bold text-foreground">
                        Simulate Clean Cleared Pass:
                      </p>
                      <Button
                        size="sm"
                        variant="outline"
                        className="w-full justify-start text-xs border-green-300 hover:bg-green-50 hover:text-green-700 font-mono tabular-nums"
            ...
            <TabsContent value="history">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    All historical sanctions screenings are cryptographically time-stamped and
                    logged for regulatory inspection.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {historyLoading ? (
                    <p className="text-sm text-muted-foreground">
                      Retrieving immutable audit ledger...
                    </p>
                  ) : screeningsHistory.length === 0 ? (
                    <p className="text-sm text-muted-foreground p-8 text-center bg-secondary/30 rounded-lg">
                      No screenings executed yet.
                    </p>
                  ) : (
                      ...
                      {screeningsHistory.map((h) => (
                        ...
                        >
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-muted-foreground">{h.match_details}</p>
                          ...
                          <Badge
                            variant={h.match_found ? "destructive" : "outline"}
                            className={
                              !h.match_found ? "bg-green-50 text-green-700 border-green-300" : ""
                            }
                          >
