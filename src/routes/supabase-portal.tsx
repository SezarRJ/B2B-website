...
CREATE POLICY "Sellers manage own products" ON products 
  FOR ALL USING (user_id = auth.uid());`);
      ...
      setVerifyResult({
        ...
        version:
          "PostgreSQL 15.6 on x86_64-pc-linux-gnu, compiled by gcc, 64-bit (Supabase Enterprise)",
        tables_count: 16,
        tables: [
          "users",
          "master_accounts",
          ...
          <header className="flex h-16 items-center justify-between border-b border-border bg-white px-6 lg:px-8">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-lg font-semibold text-foreground">
                Supabase Cloud Orchestration & RLS Security Suite
              </h1>
            </div>
            <Badge
              variant="outline"
            ...
            <Tabs defaultValue="verify" className="w-full">
              ...
              <TabsContent value="verify">
                <Card className="border border-border bg-white shadow-sm">
                  <CardHeader className="bg-secondary/30 border-b border-border pb-4">
                    ...
                    <CardDescription>
                      Executes automated diagnostic queries (`SELECT version()`, tables count)
                      against your configured `DATABASE_URL`.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="p-6 space-y-6">
                    {loading ? (
                      <p className="text-sm text-muted-foreground">
                        Authenticating TCP/IP network packets...
                      </p>
                    ) : (
                      <div className="space-y-6">
                        <div className="p-5 rounded-xl bg-slate-900 text-slate-100 space-y-3 font-mono tabular-nums text-xs shadow-inner">
                          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                            <span className="text-slate-400 font-bold font-sans uppercase">
                              Diagnostic Execution Output:
                            </span>
                            <Badge className="bg-green-500/20 text-green-300 border-green-500">
                              Handshake Secure (TCP 5432)
                            </Badge>
                          </div>
                          <div>
                          ...
                          <div>
                            <span className="text-slate-500 block text-[10px]">
                              Application Bounded Schema Tables ({verifyResult?.tables_count}):
                            </span>
                            <div className="flex flex-wrap gap-1.5 pt-1">
                              {verifyResult?.tables?.map((tb: string, tIdx: number) => (
                                <span
                                  key={tIdx}
                                  className="px-2 py-0.5 rounded bg-slate-800 text-primary-400 border border-slate-700 text-[11px]"
                                >
                                  {tb}
                                </span>
                              ))}
                        ...
                        <div className="p-4 rounded-xl bg-amber-50/80 border border-amber-200 text-amber-900 space-y-1.5 text-xs font-sans">
                          ...
                          <p className="leading-relaxed text-amber-900/90 text-[11px]">
                            If your hosting provider restricts outbound direct IPv6 TCP connections,
                            ensure your `DATABASE_URL` is pointing to your **Supabase Transaction
                            Pooler URL** (`*.pooler.supabase.com:6543`) rather than the direct
                            database instance. This absorbs server load and keeps FastAPI database
                            connections perfectly stable under heavy multi-client matching traffic.
                          </p>
                        </div>
                      </div>
                    )}
              ...
              <TabsContent value="rls">
                <Card className="border border-border bg-white shadow-sm">
                  <CardHeader className="bg-secondary/30 border-b border-border pb-4">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <CardDescription>
                          Zero-trust PostgreSQL security layer ensuring counterparties can never
                          access unauthorized trade ledgers or KYC proofs.
                        </CardDescription>
                      </div>
                      <Button
                        size="sm"
                        className="bg-primary hover:bg-primary/90 text-white"
                        onClick={() => setExecutingSql(true)}
                  ...
                  <CardContent className="p-6 space-y-6">
                    {executingSql && (
                      <div className="p-4 bg-green-50 border border-green-200 text-green-800 rounded-xl text-xs font-mono tabular-nums flex items-center gap-2">
                        <CheckCircle2 className="h-4 w-4 text-green-600 flex-shrink-0" />
                        <span>
                          RLS Migration Script (`20260619_rls_policies.sql`) executed successfully
                          across all 16 tables.
                        </span>
                      </div>
                    )}
                    ...
                    <div className="space-y-2">
                      <span className="text-xs font-bold text-foreground block">
                        Pristine SQL Policy Specification Snippet:
                      </span>
                      <pre className="p-4 rounded-xl bg-slate-900 text-slate-100 font-mono tabular-nums text-xs overflow-x-auto max-h-[450px] leading-relaxed shadow-inner">
                    ...
                    <div className="grid md:grid-cols-3 gap-4 pt-2">
                      <div className="p-4 rounded-xl bg-secondary/40 border space-y-1 text-xs">
                        <span className="font-bold text-foreground block flex items-center gap-1.5">
                          <CheckCircle2 className="h-3.5 w-3.5 text-primary" /> `auth.uid()`
                          Isolation
                        </span>
                        <p className="text-muted-foreground text-[11px] leading-relaxed">
                          Every single `SELECT`, `UPDATE`, and `DELETE` automatically filters rows
                          matching the authenticated user's exact Supabase JWT Subject ID.
                        </p>
                      </div>
                      ...
                      <div className="p-4 rounded-xl bg-secondary/40 border space-y-1 text-xs">
                        ...
                        <p className="text-muted-foreground text-[11px] leading-relaxed">
                          `products` and `demands` tables bypass strict ownership solely for active
                          items (`is_available = true`) --- enabling our AI matching engine to
                          discover cross-border matches.
                        </p>
                      </div>
                      ...
                      <div className="p-4 rounded-xl bg-secondary/40 border space-y-1 text-xs">
                        ...
                        <p className="text-muted-foreground text-[11px] leading-relaxed">
                          `kyc_verifications` and `sanctions_screenings` grant full write/audit
                          permissions to accounts possessing the cryptographic `'admin'` JWT claim.
                        </p>
                      </div>
                    </div>
                  </CardContent>
              ...
              <TabsContent value="auth">
                <Card className="border border-border bg-white shadow-sm">
                  <CardHeader className="bg-secondary/30 border-b border-border pb-4">
                    ...
                    <CardDescription>
                      Demonstrating how TanStack React Router's `beforeLoad` middleware inspects the
                      Supabase JWT active user session prior to rendering authenticated dashboard
                      routes.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="p-6 space-y-6">
                    ...
                    <div className="space-y-2">
                      <span className="text-xs font-bold text-foreground block uppercase font-mono tabular-nums tracking-wider">
                        Active User Session State (`getSupabaseSession()`):
                      </span>
                      <div className="p-5 rounded-xl bg-slate-900 text-slate-100 font-mono tabular-nums text-xs space-y-3 shadow-inner">
                        <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                          <span className="text-white font-bold">
                            {sessionData?.user?.email || "No session (Logged out)"}
                          </span>
                          <Badge
                            className={
                              sessionData?.user
                                ? "bg-green-500 text-white font-mono tabular-nums"
                                : "bg-red-500 text-white font-mono tabular-nums"
                            }
                          >
                            {sessionData?.user
                              ? "Session Active (HTTP 200)"
                              : "Route Blocked (Redirects to /login)"}
                          </Badge>
                        </div>

                        <div>
                          <span className="text-slate-500 block text-[10px]">
                            Decoded JWT Subject ID (`sub`):
                          </span>
                          <span className="text-primary-300 font-bold">
                            {sessionData?.user?.id || "N/A"}
                          </span>
                        </div>
                        <div>
                          <span className="text-slate-500 block text-[10px]">
                          ...
                          <pre className="text-[11px] text-slate-300 pt-0.5">
                            {JSON.stringify(
                              sessionData?.user?.user_metadata || { role: "anon" },
                              null,
                              2,
                            )}
                          </pre>
                    ...
                    <div className="space-y-2 pt-2 border-t border-border">
                      <span className="text-xs font-bold text-foreground block">
                        TanStack Router Dashboard Route Safeguard Snippet
                        (`src/routes/dashboard.tsx`):
                      </span>
                      <pre className="p-4 rounded-xl bg-slate-900 text-slate-100 font-mono tabular-nums text-xs overflow-x-auto leading-relaxed shadow-inner">
