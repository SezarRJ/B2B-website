...
function MicroservicesSpecPage() {
  const services = [
    ...
    },
    {
      ...
      endpoints: [
        "GET /api/ml-analytics/feature-weights",
        "GET /api/ml-analytics/price-predictions",
        "POST /api/ml-analytics/simulate",
      ],
          ...
          <div className="p-6 lg:p-8 max-w-6xl mx-auto space-y-8">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground max-w-3xl leading-relaxed">
                      To maintain flawless reliability and ultra-low latency as transaction volumes
                      scale across the Iraq-Iran-Turkey-EU corridors, our underlying FastAPI
                      monolithic engine is structured to cleanly decouple into seven highly
                      specialized, domain-driven microservices orchestrated by Docker Compose and
                      Kubernetes.
                    </p>
                  </div>
                </div>
              ...
              <CardHeader className="bg-slate-800/80 border-b border-slate-700/80 pb-4">
                <div className="flex items-center justify-between">
                  ...
                  <Badge className="bg-primary text-white font-mono tabular-nums text-[10px]">
                    Zero-Trust mTLS Backbone
                  </Badge>
                </div>
              </CardHeader>
              <CardContent className="p-6 space-y-6 text-xs font-mono tabular-nums">
                <div className="flex flex-col md:flex-row items-stretch justify-between gap-4 text-center">
                  ...
                  <div className="p-4 rounded-xl bg-amber-500/20 border-2 border-amber-500 text-amber-200 flex flex-col justify-center flex-1 shadow-md">
                    <span className="font-bold text-white block font-sans">
                      Redis / Kafka Asynchronous Bus
                    </span>
                    <span className="text-[10px] text-amber-300 mt-1 block">
                      Pub/Sub Backbone • ARQ Tasks
                    </span>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700/80 text-slate-300 text-[11px] leading-relaxed font-sans">
                  <strong className="text-white font-bold block mb-1">
                    Decoupled Operational Strategy:
                  </strong>
                  All internal inter-service communication requests authenticate via high-speed
                  asynchronous gRPC and REST sidecars. Database layers are strictly isolated per
                  bounded context — ensuring that intense AI matching matrices or compliance sweeps
                  never degrade core escrow settlement capabilities.
                </div>
              </CardContent>
            </Card>
            ...
            <div className="space-y-4">
              <h3 className="text-xl font-bold text-foreground">
                Domain-Driven Microservices Ledger
              </h3>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {services.map((svc) => {
                  ...
                  return (
                    <Card
                      key={svc.id}
                      className="border border-border bg-white shadow-sm flex flex-col justify-between hover:border-primary/50 transition-colors"
                    >
                      <CardHeader className="space-y-3 pb-4 border-b border-border bg-secondary/20">
                        <div className="flex items-center justify-between">
                          ...
                          <Badge
                            variant="outline"
                            className="font-mono tabular-nums text-[10px] bg-white text-primary border-primary"
                          >
                            Port {svc.port}
                          </Badge>
                        </div>
                        ...
                        <span className="text-[11px] font-mono tabular-nums text-muted-foreground block tracking-tight">
                          Stack: {svc.tech}
                        </span>
                      </CardHeader>
                      ...
                      <CardContent className="space-y-4 pt-4 flex-1 flex flex-col justify-between text-xs">
                        ...
                        <div className="space-y-3 pt-2">
                          {/* Sample endpoints */}
                          <div className="space-y-1">
                            <span className="font-bold text-foreground block uppercase text-[10px] tracking-wider font-mono tabular-nums">
                              Sample Routers:
                            </span>
                            <div className="p-2.5 bg-slate-900 text-slate-100 rounded-lg space-y-1 font-mono tabular-nums text-[11px] overflow-x-auto">
                              {svc.endpoints.map((ep, eIdx) => (
                                <div key={eIdx} className="truncate">
                                  {ep}
                                </div>
                              ))}
                            </div>
                          ...
                          <div className="p-2.5 bg-secondary/50 rounded-lg border border-border text-[11px] font-mono tabular-nums text-muted-foreground">
