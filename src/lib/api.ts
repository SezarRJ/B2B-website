...
const DEMO_PRODUCTS: Product[] = [
  ...
  },
  {
    id: 102,
    user_id: 3,
    name: "HMS 1/2 Heavy Melting Steel Scrap",
    description: "Heavy melting steel scrap, 80:20 mix. Cleaned and prepared for EAF.",
    category: "steel_scrap",
    price: "380.00",
    quantity: 200,
    unit: "ton",
    origin: "Iran",
    location: "Bandar Abbas Maritime Node, Iran",
    is_available: true,
  ...
  },
  {
    id: 103,
    user_id: 1,
    name: "Rock Phosphate 30% P2O5 Fertilizer Raw",
    description: "High-grade raw phosphate for agricultural fertilizer manufacturing.",
    category: "phosphate",
    price: "180.00",
    quantity: 1000,
    unit: "ton",
    origin: "Iraq",
    location: "Baghdad Rail Hub, Iraq",
    is_available: true,
    ...
    payments: [
      {
        ...
        currency: "USD",
        status: "held",
        processor: "garanti_bbva",
        processor_transaction_id: "tx_lc_mt700_991",
        created_at: "2026-06-02T00:00:00Z",
      },
    ],
  },
...
const DEMO_DPS: DocumentaryCollection[] = [
  {
    ...
    exporter_id: 3,
    importer_id: 2,
    remitting_bank: "Bank Pasargad Iran",
    collecting_bank: "Isbank Istanbul",
    amount: "3800.00",
    currency: "USD",
    status: "presented_to_importer",
    created_at: "2026-06-10T00:00:00Z",
  },
];
    ...
    events: [
      {
        ...
        location: "Customs Clearing Node — Umm Qasr Port",
        description:
          "Container passed automated phytosanitary audit. Export tariff locked under Iraq-Turkey FTA.",
      },
      {
        id: 772,
        shipment_id: 4001,
        timestamp: "2026-06-05T14:30:00Z",
        location: "Maritime GPS Waypoint — Persian Gulf Steaming Node",
        description:
          "Vessel securely underway. Military logistics escort locked. Steaming speed: 18.5 knots.",
      },
    ],
  },
];
...
const DEMO_NOTIFICATIONS: Notification[] = [
  {
    id: 6001,
    user_id: 2,
    title: "SWIFT MT700 L/C Issued & Advised",
    message:
      "Your Letter of Credit #LC-SWIFT-2026-00001 has been authenticated by Garanti BBVA and Trade Bank of Iraq.",
    type: "push",
    priority: "high",
    read: false,
  ...
  },
  {
    ...
    message:
      "Our statistical market trajectory engines predict a 7.2% surge in Iraqi Basra Medjool dates over the next 30 days due to harvest supply variables.",
    type: "in_app",
    priority: "medium",
    read: false,
...
const DEMO_KYCS: KYCRecord[] = [
  {
    ...
    document_url: "https://s3.tureep.ai/kyc/iran_steel_trade_license.pdf",
  ...
  },
  {
    ...
    document_url: "https://s3.tureep.ai/kyc/global_buyer_tax_proof.pdf",
...
const DEMO_SANCTIONS: SanctionsScreening[] = [
  {
    ...
    match_found: true,
    match_details: "Matched against automated embargo list: IRAN OIL COMPANY",
    screened_at: "2026-06-10T00:00:00Z",
  ...
  },
  {
    ...
    screened_against: "Consolidated (OFAC + EU + UN)",
    match_found: false,
    match_details: "Fully cleared. Zero adverse media or SDN hits identified.",
    screened_at: "2026-06-11T00:00:00Z",
...
async function runOfflineMock(path: string, options: RequestInit = {}): Promise<any> {
  ...
  if (path.includes("/workflow/actions/") && method === "DELETE") {
    const actionId = decodeURIComponent(path.split("/").pop() || "");
    const next = workflowActions.filter((entry) => entry.id !== actionId);
    saveLocal("tureep_action_log", next);
    return { status: "deleted", id: actionId };
  }
  if (path.includes("/workflow/actions") && method === "POST") {
    const activeUser = users[1];
    const log: WorkflowActionLog = {
      id: body.id || `${body.item_type}-${body.item_id}-${Date.now()}`,
      ...
      new_status: "acted",
      acted_at: body.acted_at || new Date().toISOString(),
      source: "local",
    };
    const next = [log, ...workflowActions.filter((entry) => entry.id !== log.id)].slice(0, 100);
    saveLocal("tureep_action_log", next);
    return log;
  }
  ...
  if (path.includes("/orders/") && method === "POST") {
    ...
    const newOrd: Order = {
      ...
      items: [
        {
          id: Date.now() + 1,
          product_id: targetDeal.product_id,
          quantity: targetDeal.quantity,
          unit: "ton",
          unit_price: targetDeal.suggested_price,
          total_price: String(Number(targetDeal.suggested_price) * targetDeal.quantity),
          currency: "USD",
          fulfillment_status: "pending",
          product: targetDeal.product,
        },
      ],
      payments: [
        {
          id: Date.now() + 2,
          order_id: Date.now(),
          method: targetDeal.payment_terms || "Escrow",
          amount: String(Number(targetDeal.suggested_price) * targetDeal.quantity),
          currency: "USD",
          status: "held",
          processor: "tureep_secure_custody",
          processor_transaction_id: `esc_${Date.now()}`,
          created_at: new Date().toISOString(),
        },
      ],
    };
  ...
  if (path.includes("/orders/") && path.includes("/payments") && method === "POST") {
    if (path.includes("/action")) {
      ...
      const nextOrdStatus =
        act === "release" ? "completed" : act === "refund" ? "cancelled" : "confirmed";
      const updated = orders.map((o) =>
        o.id === oId ? { ...o, status: nextOrdStatus, payment_status: nextStatus } : o,
      );
  ...
  if (path.includes("/notifications/trigger-mock")) {
    ...
    const newNotif: Notification = {
      ...
      message:
        urlParams.get("message") || "Secure communication payload broadcasted across channels.",
  ...
  if (path.includes("/billing/create-checkout-session")) {
    ...
    const updatedUsers = users.map((u) =>
      u.id === owner.id ? { ...u, account_type: targetTier } : u,
    );
      ...
      const updated = lcs.map((item) =>
        item.id === lcId
          ? {
              ...item,
              status: nextSt,
              discrepancy_notes: act === "discrepancy" ? body.notes : item.discrepancy_notes,
            }
          : item,
      );
  ...
  if (path.includes("/logistics/shipments/") && path.includes("/events") && method === "POST") {
    ...
    const updated = shipments.map((item) =>
      item.id === shId ? { ...item, status: nextSt, events: [...item.events, newEvt] } : item,
    );
  ...
  if (path.includes("/ml-analytics/feature-weights")) {
    return {
      ...
      weights: [
        ...
        {
          feature: "Geographical Corridor Distance & Maritime Risk",
          weight: 0.22,
          category: "Logistics",
        },
        ...
        { feature: "Historical Default / Discrepancy Ratio", weight: 0.1, category: "Finance" },
      ],
    };
  }
    ...
    return [
      {
        ...
        forecast_30d: "2680.50",
        confidence_interval_low: "2590.00",
        confidence_interval_high: "2775.00",
        trend: "bullish",
      },
      {
        commodity_name: "HMS 1/2 Heavy Melting Steel Scrap 80:20 (Ton)",
        current_price: "380.00",
        forecast_30d: "365.20",
        confidence_interval_low: "350.00",
        confidence_interval_high: "385.00",
        trend: "bearish",
      },
      {
        ...
        forecast_30d: "192.00",
        confidence_interval_low: "185.00",
        confidence_interval_high: "200.00",
        trend: "bullish",
      },
    ];
  }
    ...
    return [
      {
        ...
        imbalance_ratio: "1.49",
        recommended_action:
          "High Imbalance (Supply Deficit). Recommendation: Alert Silver/Gold Master Accounts to list date/crop inventory.",
      },
      {
        ...
        recommended_action:
          "Moderate Imbalance (Supply Surplus). Recommendation: Dynamic Price Markdown suggested for Steel Scrap entities.",
      },
    ];
  }
  if (path.includes("/ml-analytics/simulate")) {
    ...
    return {
      ...
      ai_node_verdict:
        adjScore >= 80
          ? "Optimal Cross-Border Route Locked"
          : "Sub-Optimal Route (High Overhead Warning)",
    };
  }
  ...
  if (path.includes("/kyc/") && path.includes("/review")) {
    ...
    const updated = kycs.map((item) =>
      item.id === kId
        ? { ...item, status: targetStatus, rejection_reason: body.rejection_reason }
        : item,
    );
  ...
  if (path.includes("/sanctions/screen")) {
    ...
    const isHit =
      upperName.includes("IRAN OIL") ||
      upperName.includes("BANK OF IRAN") ||
      upperName.includes("SANCTION");
    const newS: SanctionsScreening = {
      ...
      match_found: isHit,
      match_details: isHit
        ? `Matched against automated SDN embargo list: ${body.entity_name}`
        : "Entity fully cleared. Zero SDN matches or adverse media records identified.",
